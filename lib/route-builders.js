...
function routeBuilder(actionType, options) {
  ...
  const authEnabled = options.auth && options.auth.enabled;
  ...
  const route = {
    ...
    options: {
      ...
      response: responseBuilder(options.collection.name, options.schema, authEnabled),
    },
  };

  if (authEnabled) {
    const scope = getScope(actionType, options);

    route.options.auth = { access: { scope } };
    route.options.plugins = { 'hapi-swagger': { security: [{ oauth2: scope || [] }] } };
  }
