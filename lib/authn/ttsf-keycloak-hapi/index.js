...
class SessionGrantStore {
  constructor(options = {}) {
    this.options = Hoek.applyToDefaults({ key: 'kc_auth_grant' }, options);
    this.name = 'session';
  }
...
const createPrincipalResource = (principal) => {
  ...
  const formattedPrincipal = {
    name,
    scope,
    accessToken: { value: accessToken.token, content: accessToken.content },
  };

  if (idToken) {
    formattedPrincipal.idToken = { value: idToken.token, content: idToken.content };
  }
          ...
          if (credentials) {
            ...
            if (keycloak.config.shouldRedirectUnauthenticated(request)) {
              ...
              const loginUrl = urljoin(
                keycloak.getLoginUrl(keycloak.getLoginRedirectUrl(request)),
                locale ? `?kc_locale=${locale}` : ''
              );
    ...
    handler: async (request, reply) => {
      ...
      if (!request.query.auth_callback) {
        ...
        if (request.query.error) {
          const errorMessage = `Unable to authenticate - ${request.query.error}. ${
        ...
        try {
          ...
          const grant = await keycloak.obtainGrantFromCode(
            request.query.code,
            redirectUrl,
            request.yar.id,
            keycloak.getBaseUrl(request)
          );
          grantStore.saveGrant(request, grant);
          log(
            ['debug', 'keycloak'],
            `Access token has been successfully obtained from the authorization code:\n${grant}`
          );
          return reply.redirect(keycloak.getBaseUrl(request));
        } catch (err) {
